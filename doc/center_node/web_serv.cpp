...
//        printf("message: %s\n",message);
    ...
    storeIntoDB(req_line);
    printf("store into DB success!\n");
...
//    char tag = 'tag';
    ...
    vector<string> node_infos = splitStr(data, '&');

    char sql[200];
//    int node_id = node_infos[0];
//    double total_load = node_infos[1], cpu = node_infos[2], mem = node_infos[3];
//    double disk = node_infos[4], net = node_infos[5];
    sprintf(sql,"insert into node_info (node_id,total_load,cpu,mem,disk,net) values(%s,%s,%s,%s,%s,%s);",
            node_infos[0].c_str(),node_infos[1].c_str(),node_infos[2].c_str(),
            node_infos[3].c_str(),node_infos[4].c_str(),node_infos[5].c_str());
    printf("sql: %s\n",sql);
    MysqlTools m_tools;
    m_tools.insert(sql);
}
