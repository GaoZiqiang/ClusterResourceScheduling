...
*************************************************/
int loadBalance::weightedLeastConnection(vector<pair<int,pair<int, double>>> node_infos) {

    /* node_job_load_map应作为输入参数
     *
     * node_job_load_map结构：map{int,vector<int, double>}--{node_id,{job_num,node_load}}
     * */
...
//        int node_id = it->first;// node id
//        auto sub_it = it->second.begin();
//        int node_job_num = sub_it->first;// node job num
//        // printf("node_job_num:%d\n",node_job_num);
//        double node_load = sub_it->second;// node load
//        // printf("node_load:%f\n",node_load);
...
//    }
    ...
    // 现在不需要对输入参数进行格式转换了
    /*2 根据各节点的作业数进行排序*/
    sort(node_infos.begin(), node_infos.end(), cmp_by_job);  // vector根据cmp的规则进行排序
