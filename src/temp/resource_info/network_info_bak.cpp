...
void calNetworkInfo() {
    ...
    long long sleep_time = 10000;// 延迟时间：1000毫秒
    usleep(sleep_time);
    ...
    printf("sizes:%f\n",sizes);
    /*计算网速，单位为MiB/s*/
    double cur_rate = (double)(((long long)(inSize2 + outSize2 - inSize1 - outSize1) * 8) / (1000000 * interval));

    printf("cur_rate:%f\n",cur_rate);
    double netUsage = cur_rate / totalBandWidth;
